...
class DiffImageGeneration:
    def __init__(self):
        self.device = get_default_device()
        self.sd = SimpleDiffusion(TrainingConfig.TIMESTEPS, TrainingConfig.IMG_SHAPE, self.device)

        # 'models/diffusion', done like this for windowsers
        self.models_dict = os.path.join('models', 'diffusion')
        self.load_models()
        self.active_model = self.models[0]

    def load_models(self):
        self.models = []
        for model_name, checkpiont_path in DIFF_MODELS_CONFIG.items():
            ...
            diff_model = DiffModel(model_name, model)
            self.models.append(diff_model)

    def set_callbacks(self, step_callback, finished_callback):
        self.step_callback = step_callback
        self.finished_callback = finished_callback
        
    def switch_active_model(self, index):
        self.active_model = self.models[index]
    ...
    def generate_image(self):
        ...
        for time_step in tqdm(iterable=reversed(range(1, timesteps)), 
            ...
            self.step_callback(iter, timesteps)
            iter += 1
            ...
            self.finished_callback(pil_image)
